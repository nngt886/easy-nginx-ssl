...
install_domain() {
    ...
    # --- 核心修改：无论模式 1 还是 2，都允许自定义端口 ---
    echo ""
    read -p "📌 请输入 Nginx 监听端口（即 Cloudflare 回源端口，例如 80、443、8443、55555）: " LISTEN_PORT
    ...
    if [ "$LISTEN_PORT" != "80" ] && [ "$LISTEN_PORT" != "443" ]; then
        echo "ℹ️ 由于监听端口非标准端口，请在 Cloudflare 面板配置 Origin Rules，将回源端口设置为 ${LISTEN_PORT}"
    fi
...
EOF
    ...
    # 只有在使用 SSL 且端口不是 80 时，添加 80 跳转（保持原逻辑）
...
EOF
    ...
    echo "[5/5] 注意：端口已限制仅允许 Cloudflare IP 访问"
}
